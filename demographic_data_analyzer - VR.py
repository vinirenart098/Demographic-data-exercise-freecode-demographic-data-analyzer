...
def calculate_demographic_data(print_data=True):
    ...
    dataset_Male = df[df['sex'] == 'Male']
  
    average_age_men = round(dataset_Male['age'].mean(), 1)
    ...
    rich_percentage = round(((((df['hours-per-week'] == min_work_hours) & (df['salary'] == '>50K')).sum()) / num_min_workers) * 100, 1)
    ...
    dataset_popular_occupation_India = df[(df['salary'] == '>50K') & (df['native-country'] == 'India')].groupby('occupation').count()['salary'].sort_values(ascending=False)

    top_IN_occupation = dataset_popular_occupation_India.head(1).index[0]
